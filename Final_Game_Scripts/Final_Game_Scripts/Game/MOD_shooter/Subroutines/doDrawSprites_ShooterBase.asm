...
;; Due to that, it must be included in the same bank with luts. 
	...
	doDrawSpritesLoop:
			...
			; ------------------------------------------------
			; PLAYER COLOUR VISUAL OVERRIDE (PALETTE SWITCH)
			; ------------------------------------------------

			LDA Object_type,x
			CMP #$00        ; <<< CHANGE THIS IF YOUR PLAYER IS NOT $00
			BNE +skipPlayerPalette

				; apply palette (bits 0-1 only)
				LDA tempD
				AND #%11111100      ; clear palette bits
				ORA playerPalette   ; inject player palette
				STA tempD

			+skipPlayerPalette:	;; increasing again sets us up for the next sprite.
